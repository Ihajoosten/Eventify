...
namespace Eventify.Infrastructure.EFRepositories
{
    public class SpeakerRepository : EFRepository<Speaker>, ISpeakerRepository
    {
        ...
        {
            return await _unitOfWork.Set<Speaker>()
                .Join(_unitOfWork.Set<Session>(), speaker => speaker.Id, session => session.SpeakerId, (speaker, session) => new { Speaker = speaker, Session = session })
